      ...
      <div className=' flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
          ...
          messages?.map((msg,index)=>(
            <div key={index} className={`flex items-end justify-end gap-2
             ${msg.senderId !== authUser._id && 'flex flex-row-reverse'}`}>
                {
                  msg.image? (
                    <img src={msg.image} alt="" srcset="" className='max-w-xs border border-gray-700 
                    rounded-lg overflow-hidden mb-8'/>
                  ):(
                    <p className={`p-2 max-w-xs md:text-sm font-light rounded-lg mb-8 break-all
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
          <div className='flex flex-1 items-center bg-gray-100/12 px-3 rounded-full'>
              ...
              type="text" name="" id="" placeholder='send a message'
