    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
        ...
        justify-between max-sm:flex-col-reverse rounded-lg'>
          <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
            ...
            type='text'  placeholder='your name' className='p-2 border bg-gray-500 rounded-md focus:outline-none 
            focus:ring-2 focus:ring-violet-500' required/>

            <textarea onChange={(e)=>setBio(e.target.value)} value={bio}
            placeholder='write profile bio' className='p-2 border bg-gray-500 rounded-md focus:outline-none 
          ...
          src={authUser?.profilePic || assets.logo_icon} alt="" srcset="" />
        </div>
    </div>
  )
}
